...
(function () {
  ...
  const Timer = (() => {
    ...
    function workToggle() {
      if (currentState !== IDLE_STATE && currentTaskId === null) {
  ...
  const LogManager = (() => {
    ...
    async function logPomodoro() {
      ...
      try {
        ...
        // Also increment the selected task's pomodoro count
        if (currentTaskId) {
            await TaskManager.incrementPomodoro(currentTaskId);
        }
  ...
  function updateNotificationButton() {
    if (!('Notification' in window)) {
      ...
      enableNotificationsBtn.style.cursor = 'pointer';
      enableNotificationsBtn.style.backgroundColor = '';
      enableNotificationsBtn.style.color = '';
      notificationPermission = Notification.permission;
      if (notificationPermission === 'granted') {
        enableNotificationsBtn.textContent = "Notifications Enabled";
        enableNotificationsBtn.style.backgroundColor = 'var(--break-color)';
        enableNotificationsBtn.style.color = 'var(--light-text)';
      } else if (notificationPermission === 'denied') {
        enableNotificationsBtn.textContent = "Notifications Blocked";
        enableNotificationsBtn.disabled = true;
        enableNotificationsBtn.style.backgroundColor = 'var(--danger-color)';
        enableNotificationsBtn.style.color = 'var(--light-text)';
        enableNotificationsBtn.style.opacity = '0.6';
